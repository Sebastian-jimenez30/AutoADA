...
def ejecutar_buscar_key(app):
    ...
    if invalidas:
        messagebox.showerror("Error", f"Keys inválidas:\n{', '.join(invalidas)}")
        return
    if not keys_validas:
        messagebox.showwarning("Aviso", "No hay keys válidas.")
        return
    ...
    if not ready:
        ...
        msg = (
            ...
            "Se realizará actualización de BD obligatoria.\n"
    ...
    def _fin_buscar(rc: int):
        def _end():
            ...
            if rc == 0:
                app.success_status("Búsqueda lista")
                if os.path.exists(output_file):
                    mensaje = (
                        "Búsqueda de claves completada exitosamente.\n\n"
                        f"Se encontraron {len(keys_validas)} claves procesadas."
                    )
                    show_success_with_open(
                        parent=app.ventana,
                        mensaje=mensaje,
                    ...
                    messagebox.showinfo("Proceso listo", "Búsqueda lista. Revisa la consola.", parent=app.ventana)
            else:
    ...
    app.start_status("Preparando búsqueda...", indeterminate=True)
    if getattr(app, "console", None):
        app.console.clear()
