...
def ejecutar_crear_tag_hsh(app, aplicar: bool = False, origin: str | None = None):
    ...
    # ---------- SCADA bits (igual que antes; no lo toco si ya te funciona) ----------
    ...
    def _finish(rc: int, error_message: str | None = None, verification_summary: Optional[List[str]] = None):
        def _end():
            ...
            if rc == 0:
                ...
                try:
                    if hasattr(app, "refresh_last_upd_hsh_crear"): app.refresh_last_upd_hsh_crear()
                except Exception: pass
                archivos = [p for p in [*report_paths, *extra_paths] if os.path.exists(p)]
                mensaje = f"Archivo validado para {empresa}." + (f"\nIncluye revision con respaldo {respaldo}." if respaldo else "")
                if aplicar:
                    mensaje += "\nTags insertados en HSH." if archivos else "\nRevisa la consola para confirmar la insercion."
                    if verification_summary: mensaje += "\n\n" + "\n".join(verification_summary)
                if archivos:
                    show_success_with_open(parent=app.ventana, mensaje=mensaje, title="Proceso listo", files=archivos)
                else:
                    messagebox.showinfo("Proceso finalizado", mensaje, parent=app.ventana)
            else:
                app.error_status("El proceso termino con errores")
                msg = error_message or "El proceso termino con errores. Revisa la consola."
                if verification_summary: msg += "\n\n" + "\n".join(verification_summary)
                messagebox.showerror("Error", msg, parent=app.ventana)
        _ui(_end)
    ...
    def _start_verification():
        ...
        if not verify_queue:
            if aplicar:
                ...
                if scada_failed: verification_failed = True; _complete(1); return
                if inserted_keys_map:
                    _console("[PI] esperando 10 segundos antes de consultar valores", "info"); time.sleep(10)
                pi_lines, pi_failed = _collect_pi_snapshots()
                if pi_failed: verification_failed = True; _complete(1); return
            _complete(0); return
        ...
        def _verify_next():
            ...
            if not verify_queue:
                if aplicar:
                    scada_lines, scada_failed = _apply_scada_updates()
                    if scada_failed: verification_failed = True
                    else:
                        pi_lines, pi_failed = _collect_pi_snapshots()
                        if pi_failed: verification_failed = True
        ...
        def _after_convert_verify(rc: int, emp: str, keys: List[str]):
            ...
            if missing:
                faltan_txt = ", ".join(_base_keys(missing))
...
__all__ = ["ejecutar_crear_tag_hsh"]
