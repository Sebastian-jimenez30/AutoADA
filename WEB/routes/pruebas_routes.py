...
@router.get("/pruebas/itcosas-v2", response_class=HTMLResponse, name="pruebas_itcosas_v2_page")
def pruebas_itcosas_v2_page(request: Request):
    context = {
        "request": request,
        "active_section": "pruebas",
        "active_page": "pruebas_itcosas_v2",
        "page_title": "Pruebas - ITCOSAS v2",
        "page_subtitle": "Ejecuta el pipeline de ITCOSAS v2 (SOE Local v2, HIS, SOE Monarch v2, Checklist v2).",
        "empresas": list(pruebas_controller.get_empresas()),
        "dominios": list(pruebas_controller.get_dominios()),
    }
    return templates.TemplateResponse("pruebas_itcosas_v2.html", context)
...
@router.post("/pruebas/itcosas-v2/run")
def ejecutar_pruebas_itcosas_v2(
    ...
    hora_fin: str = Form(...),
    checklist: UploadFile = File(...),
    soe_se: UploadFile = File(...),
):
    uploads = {"checklist": checklist, "soe_se": soe_se}
    stored: dict[str, str] = {}
    os.makedirs(pruebas_controller.PRUEBAS_UPLOAD, exist_ok=True)
    try:
        for key, uf in uploads.items():
...
@router.get("/pruebas/itcosas-v2/result")
def obtener_pruebas_itcosas_v2_result(
    sheet: str | None = Query(None),
    limit: int = Query(500, ge=1, le=5000),
):
    data = pruebas_controller.load_pruebas_itcosas_v2_preview(sheet=sheet, limit=limit)
    if data is None:
        raise HTTPException(status_code=404, detail="No hay resultados disponibles.")
    return data
...
@router.get("/pruebas/itcosas-v2/result/download")
def descargar_pruebas_itcosas_v2_result(path: str):
    if not path:
        raise HTTPException(status_code=400, detail="Ruta inválida.")
    file_path = Path(path)
    if not file_path.is_absolute():
